...
bool PN532::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    ...
    if (HAL(writeCommand)(pn532_packetbuffer, 2, send, sendLength)) {
        DMSG("writeCommand an error\n");
        return false;
    }

    int16_t status = HAL(readResponse)(response, *responseLength, 2000);
    if (status < 0) {
        DMSG("readResponse status:");
        DMSG_HEX(status);
        DMSG("readResponse an error\n");
        return false;
    }
...
bool PN532::inListPassiveTarget()
{
    ...
    int16_t status = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), 2000);
    if (status < 0) {
        return false;
    }
