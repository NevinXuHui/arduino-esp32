...
bool EmulateTag::emulate(const uint16_t tgInitAsTargetTimeout){
  Serial.println("EmulateTag::emulate:1");

  uint8_t command[] = {
        PN532_COMMAND_TGINITASTARGET,
        4,                  // MODE: PICC only, Passive only
        ...
        // FELICA PARAMS
              // NFCID2t (8 bytes) https://github.com/adafruit/Adafruit-PN532/blob/master/Adafruit_PN532.cpp FeliCa NEEDS TO BEGIN WITH 0x01 0xFE!
      0x02,0xFE,0x05, 0x01, 0x86,
      0x04, 0x02, 0x02,
...
Serial.println("EmulateTag::emulate:2");
...
Serial.println("EmulateTag::emulate:3");
  ...
  if(p2 != 0x0c){
    ...
    if(rwbuf[C_APDU_DATA+1] == 0x03){
      currentFile = CC;
      Serial.println("currentFile = CC;");
    } else if(rwbuf[C_APDU_DATA+1] == 0x04){
      currentFile = NDEF;
      Serial.println("currentFile = NDEF;");
    }
  } else {
  ...
  if( p1p2_length > NDEF_MAX_LENGTH){
    ...
    setResponse(COMMAND_COMPLETE, rwbuf, &sendlen);
    tagWrittenByInitiator = true;
    Serial.println("tagWrittenByInitiator:111");
